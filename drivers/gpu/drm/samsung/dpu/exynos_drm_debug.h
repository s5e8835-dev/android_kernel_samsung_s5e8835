...
/**
 * STATE_FMT - printf string for &struct drm_crtc_state
 */
#define STATE_FMT "enable(%d) active(%d) change[p:%d m:%d a:%d conn:%d color:%d]"
/**
 * STATE_ARG - printf arguments for &struct drm_crtc_state
 * @s: drm_crtc_state struct
 */
#define STATE_ARG(s) (s)->enable, (s)->active, (s)->planes_changed,	\
	(s)->mode_changed, (s)->active_changed, (s)->connectors_changed,\
	(s)->color_mgmt_changed

/**
 * FREQ_FMT - printf string for &struct dpu_bts to print mif, int and disp clk
 */
#define FREQ_FMT "mif(%lu) int(%lu) disp(%lu)"
/**
 * FREQ_ARG - printf arguments for &struct dpu_bts to print mif, int and disp clk
 * @b: dpu_bts struct
 */
#define FREQ_ARG(b)						\
...
void exynos_atomic_commit_check_buf_sanity(struct drm_atomic_state *old_state);
#else
static inline void
exynos_atomic_commit_prepare_buf_sanity(struct drm_atomic_state *old_state) { }
static inline void
